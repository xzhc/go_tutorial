...
func Hello(name string) (string, error) {
	...
	// message := fmt.Sprint(randomFormat()) // testingfailed example
	return message, nil
}
...
func Hellos(names []string) (map[string]string, error) {
	...
	//Loop through the received slice of names, calling the Hello function to get a message for each time
	for _, name := range names {
		message, err := Hello(name)
		if err != nil {
			return nil, err
		}
		//In the loop, associate the retrieved message with the name
